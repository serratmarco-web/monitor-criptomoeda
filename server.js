...
// Endpoints que o push service respondeu 404/410 (inscrição morta). Se o app
// reenviar um deles, o servidor pede para ele criar uma inscrição nova.
const deadEndpoints = new Set();
function dropDead(clientId) {
  const ep = subscribers[clientId]?.subscription?.endpoint;
  if (ep) deadEndpoints.add(ep);
  delete subscribers[clientId];
  saveSubscribers();
  console.log(`[-] Inscrição expirada removida: ${clientId}`);
}
...
app.get('/', (req, res) => res.json({
  ...
  lastCheck: lastCheckAt ? new Date(lastCheckAt).toISOString() : null,
  secondsSinceLastCheck: lastCheckAt ? Math.floor((Date.now() - lastCheckAt) / 1000) : null,
  lastError,
  uptime: Math.floor(process.uptime()) + 's'
}));

// Diagnóstico: o que o servidor sabe sobre você (sem expor a subscription)
app.get('/debug/:clientId', (req, res) => {
  const d = subscribers[req.params.clientId];
  if (!d) return res.status(404).json({ error: 'clientId não encontrado — abra o app para registrar', subscribers: Object.keys(subscribers).length });
  res.json({ lastPrice, alerts: d.alerts, variations: d.variations });
});

// Para um cron externo (cron-job.org) chamar a cada 5 min: acorda o servidor e força a verificação
app.get('/check', async (req, res) => {
  await checkAndNotify();
  res.json({ ok: true, lastPrice, priceSource: lastSource, lastError, subscribers: Object.keys(subscribers).length });
});
...
app.post('/subscribe', (req, res) => {
  ...
  if (deadEndpoints.has(subscription.endpoint)) {
    return res.json({ ok: false, resubscribe: true });
  }
...
// CoinGecko costuma bloquear (429) IPs de servidores na nuvem, como os do Render.
// Por isso há fontes de reserva: tenta em ordem até uma responder.
const PRICE_SOURCES = [
  ['coingecko',      'https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=brl', d => d?.ethereum?.brl],
  ['coinbase',       'https://api.coinbase.com/v2/prices/ETH-BRL/spot',                              d => parseFloat(d?.data?.amount)],
  ['mercadobitcoin', 'https://www.mercadobitcoin.net/api/ETH/ticker/',                                d => parseFloat(d?.ticker?.last)]
];

async function fetchETHPrice() {
  const errors = [];
  for (const [name, url, pick] of PRICE_SOURCES) {
    try {
      const res = await fetch(url, { headers: { 'User-Agent': 'eth-monitor/1.0', Accept: 'application/json' } });
      if (!res.ok) throw new Error('HTTP ' + res.status);
      const price = pick(await res.json());
      if (!price || !isFinite(price)) throw new Error('resposta sem preço');
      lastSource = name;
...
async function checkAndNotify() {
  ...
  try {
    ...
    lastPrice  = price;
    console.log(`[ETH] R$ ${price.toLocaleString('pt-BR')} [${lastSource}] ${prev ? `(antes: R$ ${prev.toLocaleString('pt-BR')})` : '(primeiro fetch)'}`);
    ...
    for (const [clientId, data] of Object.entries(subscribers)) {
      ...
      for (const alert of (data.alerts || [])) {
        ...
        const payload  = {
          title: `🚨 ETH ${alert.direction === 'above' ? '📈' : '📉'} ${alert.name || 'Alerta atingido!'}`,
        ...
        try {
          ...
          if (isGone(e)) { dropDead(clientId); break; }
        }
      }
      ...
      for (const v of (data.variations || [])) {
        ...
        const payload = {
          title: `🚨 ETH ${dirIcon} ${v.name || 'Faixa rompida!'}`,
...
app.get('/test-push/:clientId', async (req, res) => {
  ...
  try {
    ...
    if (isGone(e)) {
      dropDead(req.params.clientId);
      return res.status(410).json({ error: 'Inscrição expirada — abra o app no celular para renovar automaticamente e teste de novo.' });
    }
